...
public class InternationalPayTest {
	...
	static void testInternationalPay() {
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(RESULT_TYPE.PARAM_INVALID.name()));
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(TestConstant.CURRENCY_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue("",
					ex.getMessage()
							.contains(TestConstant.CREDIT_CARD_INFO_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue("",
					ex.getMessage()
							.contains(TestConstant.CREDIT_CARD_ID_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue("",
					ex.getMessage()
							.contains(TestConstant.PAYPAL_RETURN_URL_EMPTY));
		}
		order.setChannel(channel);
	...
	private static void mockInternationalPay(BCInternationlOrder order) {
		...
		new Expectations() {
			{
				Deencapsulation.invoke(BCPay.class, "doPost",
						withSubstring(BCUtilPrivate.getApiInternationalPay()
				...
				result = new BCException(RESULT_TYPE.CHANNEL_ERROR.ordinal(),
						RESULT_TYPE.CHANNEL_ERROR.name(),
						TestConstant.MOCK_CHANNEL_ERROR_MSG);
			}
		};
		...
		try {
			...
			Assert.assertTrue(ex.getMessage(),
					ex.getMessage()
							.contains(RESULT_TYPE.CHANNEL_ERROR.name()));
			Assert.assertTrue(ex.getMessage(),
					ex.getMessage()
							.contains(TestConstant.MOCK_CHANNEL_ERROR_MSG));
		}
	}
