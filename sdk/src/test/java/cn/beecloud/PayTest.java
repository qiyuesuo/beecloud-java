...
public class PayTest {
	...
	static String refundNo = new SimpleDateFormat("yyyyMMdd")
			.format(new Date()) + BCUtil.generateNumberWith3to24digitals();
	...
	static void testPay() {
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(RESULT_TYPE.PARAM_INVALID.name()));
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(TestConstant.CHANNEL_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(RESULT_TYPE.PARAM_INVALID.name()));
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(TestConstant.BILL_NO_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(RESULT_TYPE.PARAM_INVALID.name()));
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(TestConstant.TITLE_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue("",
					!ex.getMessage()
							.contains(TestConstant.TITLE_FORMAT_INVALID));
		}
		param.setTitle(subject);
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage()
							.contains(TestConstant.BILL_NO_FORMAT_INVALID));
		}
		param.setBillNo(billNo);
		//
		...
		try {
			...
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(RESULT_TYPE.PARAM_INVALID.name()));
			Assert.assertTrue(e.getMessage(),
					e.getMessage().contains(TestConstant.OPENID_EMPTY));
		}
		...
		try {
			...
			Assert.assertTrue(
					...
					e.getMessage().contains(
							TestConstant.YEE_NOBANCARD_FACTOR_EMPTY));
		}
	...
	private static void mockWxNativePay(BCOrder param) {
		...
		System.out
				.println("urllll:" + BCUtilPrivate.getkApiPay().substring(14));

		new Expectations(BCPay.class) {
			{
				Deencapsulation
						.invoke(BCPay.class, "doPost",
								withSubstring(BCUtilPrivate.getkApiPay()
										.substring(14)), withAny(Map.class));
				returns(returnMap);
				result = new BCException(
						RESULT_TYPE.PAY_FACTOR_NOT_SET.ordinal(),
		...
		try {
			...
			Assert.assertEquals("", TestConstant.MOCK_CODE_URL,
					order.getCodeUrl());
		...
		new Expectations() {
			{
				...
				result = new BCException(RESULT_TYPE.RUNTIME_ERORR.ordinal(),
						RESULT_TYPE.RUNTIME_ERORR.name(),
						RESULT_TYPE.RUNTIME_ERORR.name());
			}
		};
	...
	private static void mockWxJsapi(BCOrder param) {
		...
		new Expectations() {
			{
				...
				result = new BCException(RESULT_TYPE.CHANNEL_INVALID.ordinal(),
						RESULT_TYPE.CHANNEL_INVALID.name(),
						RESULT_TYPE.CHANNEL_INVALID.name());
			}
		};
		BCOrder order;
		...
		try {
			order = BCPay.startBCPay(param);
			Assert.assertEquals("", TestConstant.MOCK_WXJSAPI_APPID, order
					.getWxJSAPIMap().get("appId"));
			Assert.assertEquals("", TestConstant.MOCK_WXJSAPI_NONCESTR, order
					.getWxJSAPIMap().get("nonceStr"));
			Assert.assertEquals("", TestConstant.MOCK_WXJSAP_TIMESTAMP, order
					.getWxJSAPIMap().get("timeStamp"));
			Assert.assertEquals("", TestConstant.MOCK_WXJSAPI_PAYSIGN, order
					.getWxJSAPIMap().get("paySign"));
			Assert.assertEquals("", TestConstant.MOCK_WXJSAP_SIGNTYPE, order
					.getWxJSAPIMap().get("signType"));
			Assert.assertEquals("", TestConstant.MOCK_WXJSAPI_PACKAGE, order
					.getWxJSAPIMap().get("package"));
			Assert.assertEquals("", TestConstant.MOCK_OBJECT_ID,
					order.getObjectId());
		} catch (BCException e) {
			Assert.fail(TestConstant.ASSERT_MESSAGE_BCEXCEPTION_THROWN);
		...
		try {
			...
			Assert.assertTrue(ex.getMessage(),
					ex.getMessage()
							.contains(RESULT_TYPE.CHANNEL_INVALID.name()));
		}
	}
